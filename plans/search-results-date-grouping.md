...
`groupBy=dateImported`. Explore performant queries and JSON delivery while
preserving sorting and continuous scrolling. Produce a reviewable implementation
...
ready for implementation review. Numbered units describe future changes, not work
already implemented. SQLite query experiments are complete; PostgreSQL and full
application validation remain implementation checks.
...
| Scale | 100,000 images as baseline and 1 million as stress case, including days with thousands of images and deep scrolling. |
...
| Global and per-day window counts before page slicing | Correct and concise, but much slower in the isolated SQLite experiment. Keep as a benchmark comparator for complex filters/PostgreSQL. |
...
## Indexes and performance evidence
...
require work. They do not cover every secondary field or mixed direction. Add a
secondary-sort suffix/index only when representative measurements justify its
write/storage cost. Reversing a composite index reverses all its directions, not
just the day; see [PostgreSQL index ordering](https://www.postgresql.org/docs/current/indexes-ordering.html).

The [query experiment report](search-results-date-grouping-benchmark.md) records
...
Engineering validation targets, not user-supplied SLAs: measure warm end-to-end
p50/p95 for 50-image pages, aiming for p95 below 200 ms at baseline and below 1 second
at the stress size for simple indexed searches on a documented test machine.
Report cold runs and complex-filter results separately. Test both grouping modes,
all supported sorts/directions, broad and selective filters, large individual days,
first/deep/end pages, and ingestion/index-build costs. If these targets fail,
inspect plans and adjust indexes/query shape before adding caches. A very large
single day may need a matching secondary-sort index even when ordinary days do not.
...
1. **Preserve date semantics in storage and decoding.** Add integration coverage
   ...
   PostgreSQL capture column and its mapping as described above; keep SQLite local
   capture parsing timezone-free, and bind new asset import times explicitly in
   UTC. Cover fresh and upgraded schemas, including
   preservation of existing represented values. Use the next migration version
   (currently 3; check at implementation time), both engine migration files,
   `all.sql`, and the `schemaVersion` bump. Rationale: query optimization cannot
   compensate for grouping under the wrong calendar date.
...
2. **Add the typed grouped ID-search service and count query.** Extend the search
   ...
   preserve the full grouped query. Add the two day indexes through paired
   migrations. Prove unique IDs, filtered day/global totals, independent ordering,
   empty pages, and multi-page days with integration fixtures. Rationale: one
...
5. **Validate full queries and document the implemented contract.** Run the
   integration/controller suites and reproduce representative measurements on
   SQLite and PostgreSQL with actual filter combinations and realistic row widths.
   Inspect `EXPLAIN QUERY PLAN` / `EXPLAIN (ANALYZE, BUFFERS)` and adjust only
   demonstrated bottlenecks. Update `altitude/AGENTS.md` with the implemented ID
   search path, paging, date storage, and JSON contract; update other existing
   ...
   the operational limits rather than treating prototype timings as guarantees.
...
and performance checks require an explicitly configured isolated test database;
