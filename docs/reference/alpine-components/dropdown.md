...
The folder and album context menus (`static/js/common/context-menu-markup.js` builds them for the folder tree and the album list; `static/js/alpine/components/context-menu.js` coordinates them) are native `popover="auto"` panels rather than this `x-show` dropdown: the browser owns visibility, outside-click dismissal, and the trigger relationship (`popovertarget`), while the close-on-focus-out idea from this lesson is kept in the component. Escape is handled once for the whole document in `global.js` instead of on the component root, so it works wherever focus is. Choosing an action requests a separate modal; the modal owner closes the menu as it opens.
The menu keeps its action list throughout.
