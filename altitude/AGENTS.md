...
- `static/js/common/album-list.js` — renders the flat album list client-side from `/api/album/r/:repoId/list` (`AlbumController`), with the same menu (`common/context-menu-markup.js`, shared with the folder tree) and count cell after the name (`common/asset-count.js`); `refreshAlbumCounts` patches the counts in place after membership changes and asset mutations. Albums are pointers only: see **Albums** in `views/AGENTS.md`
- `static/js/common/location-list.js` — renders the categories and Locations client-side from `/api/location/r/:repoId/list` (`LocationController`, path order) with the same menu and count cell, each category an expandable node holding its Locations (collapsed on load, expansion kept across rebuilds); `refreshLocationCounts` patches the counts in place. Membership drops and the batch footer's Add / Remove go through `assets/asset-actions.js`: see **Locations** in `views/AGENTS.md`
...
The pipeline works on **staged files**, never on byte arrays: `AssetWithData` carries the asset and the path of its file. An upload lands in `data/staging/<uuid>` (`StagingService`; `ImportController` moves Undertow's temp file there, since Undertow deletes it when the request ends, before the queued pipeline runs), a test or an `ImportAsset` stages a copy, and the checksum is streamed (`MurmurHash.hash32(path)`, the same value as over the bytes). `FileStoreFlow` renames the staged file into `files/<id[0:2]>/<id>` (`FileStoreService.assetFile(id)` exposes it), so a staged file is consumed by exactly one import; after that flow the element's path is the stored file. `AssetWithData.bytes` reads the whole file for the image-only consumers (thumbnails, `ImageIO`, face detection). An asset the pipeline drops carries its staged file in `InvalidAsset`, and `StripBinaryDataFlow`, where the file leaves the pipeline, deletes it; `Altitude` startup empties staging. `ExtractMetadataFlow` turns a `VideoException` (a container Tika calls a video but FFmpeg cannot open) into an invalid asset rather than a stream failure.

## Video

A Video is an asset whose `media_type` is `video` (there is no flag column); `LibraryService.SUPPORTED_MEDIA_TYPES` admits `image` and `video`, so an audio file stays unsupported. Decoding goes through FFmpeg by way of javacv: `build.mill` declares each bytedeco preset as its stub jar plus a `classifier=<platform>` jar with the natives of the detected platform (appending the platform to the version resolves nothing), and `VideoService` wraps `FFmpegFrameGrabber`, one grabber per call, nothing shared across threads.

- `probe(path)` gives `VideoInfo`: the display size (the container's display matrix applied, so a phone's portrait recording is stored portrait, `width`/`height` on the asset), the duration (`asset.duration_ms`, NULL for an image), frame rate and audio presence. `AssetService.getDimensions` and `getDuration` dispatch on media type.
- **Sampled frames**: `VideoService.sampleTimes(durationMs)` spaces Frame times evenly from the start at `max(video.faces.sample_interval_ms, duration / video.faces.max_sampled_frames)` (defaults 1000 ms and 120), so a short clip is sampled every second and a two-hour recording about every minute. `sampledFrames(path, times)(consume)` decodes each as an upright BGR `org.opencv.core.Mat` for an iterator that lives as long as the call; the consumer releases each image. Frames are turned the way FFmpeg's own autorotate turns them: the container's counter-clockwise display rotation, negated, snapped to a quarter turn (`VideoService.uprightRotation`).
- **Preview**: `previewFrame(path)` is the first Sampled frame whose mean brightness clears `video.preview.min_luminance` (skipping a black leader), else the frame at a tenth of the duration; `AssetService.genPreviewData` encodes it to PNG and thumbnails it like an image, so the 200 px Preview, the map and the grid are unchanged.
- **Faces**: `FaceDetectionService.extractFaces` has a `Mat` overload (the byte-array one decodes and delegates). `FaceRecognitionService.processAsset` dispatches on media type; for a Video it detects faces in every Sampled frame (each Face pinned to its Frame time, `face.frame_time_ms`, NULL for a Face in an image), clusters the detections of the whole video in descending detection score (a detection joins the first cluster whose representative is within `face.recognition.cosine_distance_threshold`, else starts one; the representative is the cluster's first detection and is what the Face stores, so the saved crops and the vector agree), and recognizes each representative through the usual top-K vote. One Face per Person per Video: when two clusters resolve to the same Person the lower-scoring one is dropped, so a pose change does not fail the import the way `SamePersonDetectedTwiceException` does for an image. Both pipelines get this through the service.
- **Dates and places**: metadata-extractor reads MP4 and QuickTime containers into `extracted_metadata`, so the resolvers still read persisted inputs only. `CaptureDateResolver` has two rungs below EXIF, XMP, IPTC and the PNG text chunk: `QuickTime Metadata` / `Creation Date` (`com.apple.quicktime.creationdate`, a local wall clock whose offset is discarded like every other; `qt_creation_date`), then, with the PNG tIME rung and above GPS and the filename, the container's `Creation Time` (`MP4 Video`, `QuickTime Video`, `MP4`, `QuickTime`; `container_creation_time`). The extractor describes that UTC instant in the JVM's zone, with a numeric offset on the track directories and a zone name on the container ones; `WallClockParser.parseUtcInstant` turns either back into the UTC wall clock, so the stored value does not depend on the server's zone, and the 1904 epoch remains a sentinel. `GeoLocationResolver` tries EXIF `GPS`, then `QuickTime Metadata` / `ISO 6709` (`+37.3318-122.0312+015.000/`, also its degrees-minutes forms), then `MP4` / `Latitude` and `Longitude`. `Asset.getPublicMetadata` takes `deviceModel` from `QuickTime Metadata` / `Model` when EXIF has none.
- **Serving**: `ContentViewController` streams `file` content from the stored file through `RangeStreaming`, a `geny.Writable` window over a `FileChannel`: `Accept-Ranges: bytes`, a `206` with `Content-Range` for a single `bytes=a-b` range (`bytes=a-` and `bytes=-n` included), `416` past the end, `200` with `Content-Length` otherwise, and `Content-Type` is the asset's stored `mime_type` for images too. `decorators.compress` is cask's gzip decorator except for a response that offers byte ranges, which is left uncompressed: a range names raw bytes.
- **Tests**: `TestVideos` synthesizes short MP4s (`FFmpegFrameRecorder`, `mpeg4`) from the `people/` images at test time, with a display rotation or container metadata on request, so no binaries live in git; `VideoServiceTests` covers decoding, `AssetImportServiceTests` the import, `FaceRecognitionServiceTests` the faces, `ContentViewControllerTests` the ranges.
