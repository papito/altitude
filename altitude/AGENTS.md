...
The contract on `GET /htmx/search/r/:repoId` is HTML only for grouped results: with `groupBy`, JSON negotiation (`Accept` or the legacy `Content-Type: application/json`) is a 400. The ungrouped JSON stays as it was.
