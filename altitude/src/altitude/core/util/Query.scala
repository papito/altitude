...
object Query:
  ...
  def IN(values: Set[Any], negate: Boolean = false): QueryParam =
    ...
    if values.size == 1 then new QueryParam(values.head, ParamType.EQ, negate)
...
class Query(val params: Map[String, Any] = Map(), val rpp: Int = 0, val page: Int = 1, val sort: List[Sort] = List()):
  ...
  if sort.size > 1 then throw new IllegalArgumentException("Only one sort currently supported")
