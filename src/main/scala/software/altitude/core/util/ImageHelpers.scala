...
object ImageHelpers {
  ...
  def resize(source: BufferedImage, previewBoxSize: Int): BufferedImage = {
    ...
    sourceColorModel match {
      case indexColorModel: IndexColorModel if sourceColorModel.hasAlpha && sourceColorModel.getTransparency == Transparency.BITMASK && indexColorModel.getTransparentPixel >= 0 =>
