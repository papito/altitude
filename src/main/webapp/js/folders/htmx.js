...
import {showErrorSnackBar} from "/js/common/snackbar.js";
...
htmx.find("#rootFolderList").addEventListener('htmx:beforeRequest', function(evt) {
     ...
     */
    if (evt.detail.pathInfo.requestPath  === `/htmx/folder/r/${context.repoId}/context-menu`) {
     ...
     */
    if (evt.detail.pathInfo.requestPath  === `/htmx/folder/r/${context.repoId}/children`) {
...
htmx.find("#rootFolderList").addEventListener('htmx:afterRequest', function(evt) {
    ...
    if (requestPath === `/htmx/folder/r/${context.repoId}/children` || requestPath === `/htmx/folder/r/${context.repoId}/add`) {
        const folder = new Folder(evt.target.getAttribute("folder-id"))
        folder.expand()
    }
});
